...
var walls = 15; // Количество препятствий
 ...
 * @returns {boolean}
 */
function isSnakeOrWallUnit(unit) { //проверка, что змейка не попала сама в себя в новой ячейке
    var check = false;

    if (snake.includes(unit) || unit.getAttribute('class').split(' ').includes('wall-unit')) { //если в змейке содержится новая ячейка, значит возникло пересечение
        check = true;
    }
    return check;
}
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        curScore = document.querySelector('.score').innerText.split(' ');
        curScore[2] = score;
        document.querySelector('.score').innerHTML = curScore.join(' ');
    }
    return check;
}
